...
fun ExpensesScreen(
    ...
    ExpensesContent(state = state, onEvent = viewModel::onEvent)
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun ExpensesContent(state: ExpensesState, onEvent: (event: ExpensesEvent) -> Unit) {
    Scaffold(
        ...
        LazyColumn(modifier = Modifier.padding(it), contentPadding = PaddingValues(4.dp)) {
            items(items = state.expenses, key = { expense -> expense.id }) { expense ->
                ListItem(
                    modifier = Modifier
                        .clickable { onEvent(ExpensesEvent.NavigateToExpense(expense.id)) }
                        .padding(4.dp)
                        .fillMaxWidth(),
